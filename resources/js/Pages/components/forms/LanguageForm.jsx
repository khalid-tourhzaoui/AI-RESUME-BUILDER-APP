...
function LanguageForm({ document, handleNext }) {
  const [languageList, setLanguageList] = useState(() => document?.languages || [initialState]);
  ...
  useEffect(() => {
    const processLanguages = async () => {
      try {
        ...
        await Promise.all(languageList.map((lang) => languageSchema.validate(lang)));
        setIsFormValid(true);
      } catch (err) {
  ...
  const addNewLanguage = () => setLanguageList([...languageList, initialState]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (toUpdate.length > 0) await put(route("profile-details.update", document.id), { languages: toUpdate });
      if (toAdd.length > 0) await post(route("profile-details.store", document.id), { languages: toAdd });
  ...
  const filteredLanguages = languages.filter(
    (language) => language.name.toLowerCase().includes(search.toLowerCase()) || language.code.toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
          ...
          {languageList.map((item, index) => (
            <div key={index}>
              ...
              {index === languageList.length - 1 && languageList.length < 15 && (
                <Button className="gap-1 mt-1 text-primary border-primary/50" variant="outline" type="button" disabled={processing} onClick={addNewLanguage}>
                  <Plus size="15px" />
                  Add More Languages
                </Button>
              )}
        ...
        <Button className="mt-4" type="submit" disabled={!isFormValid || processing}>
