...
function TrashListBox({document}) {
    ...
    const {put,post,delete: destroy,data,setData,processing} = useForm({document: documentList});
    ...
    const onClick = (docId) => {};
    const onRestore = async (event,docId,status) => {}
    return (
        <Popover>
            <PopoverTrigger>
                <Button
                    className="text-[15px] gap-[2px] items-center" variant="outline">
                    <Trash2 size="15px" />
                    <span>All Trash</span>
                </Button>
            </PopoverTrigger>
                        ...
                        <div className="mt-2 px-1 pb-1">
                            ...
                            {filteredDocuments?.map((doc) => (
                                <div
                                    ...
                                    <div>
                                        <div role="button" onClick={(e) => onRestore(e,doc.documentId,doc.status)}
                                            className="rounded-sm hover:bg-neutral-200 w-6 h-6 flex items-center
                                            justify-center dark:hover:bg-gray-700">
                                            {isPending ? (
